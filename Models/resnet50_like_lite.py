...
def identity_block(input_tensor, kernel_size, filters, stage, block, alpha=1.0):
    filters1, filters2, filters3 = filters

    conv_name_base = 'res' + str(stage) + block + '_branch'
    bn_name_base = 'bn' + str(stage) + block + '_branch'

    x = Conv2D(int(filters1 * alpha), (1, 1), name=conv_name_base + '2a')(input_tensor)
    x = BatchNormalization(name=bn_name_base + '2a')(x)
    x = Activation('relu')(x)

    x = SeparableConvolution2D(int(filters2 * alpha), kernel_size, padding='same', name=conv_name_base + '2b')(x)
    x = BatchNormalization(name=bn_name_base + '2b')(x)
    x = Activation('relu')(x)

    x = Conv2D(int(filters3 * alpha), (1, 1), padding="same", name=conv_name_base + '2c')(x)
...
def conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2, 2), alpha=1.0):
    filters1, filters2, filters3 = filters
    conv_name_base = 'res' + str(stage) + block + '_branch'
    bn_name_base = 'bn' + str(stage) + block + '_branch'

    x = Conv2D(int(filters1 * alpha), (1, 1), strides=strides, name=conv_name_base + '2a')(input_tensor)
    x = BatchNormalization(name=bn_name_base + '2a')(x)
    x = Activation('relu')(x)
    ...
    x = Conv2D(int(filters3 * alpha), (1, 1), name=conv_name_base + '2c')(x)
    x = BatchNormalization(name=bn_name_base + '2c')(x)

    shortcut = Conv2D(int(filters3 * alpha), (1, 1), strides=strides, name=conv_name_base + '1')(input_tensor)
...
def ResNet50_like(input_shape=(para.img_cols, para.img_rows, para.channels), classes=para.num_classes,
    ...
    # Block 2
    x = conv_block(x, 3, [128, 128, 256], stage=2, block='a', strides=(1, 1), alpha=alpha)
    x = identity_block(x, 3, [128, 128, 256], stage=2, block='b', alpha=alpha)
    y2 = x


    # Block 3
    x = conv_block(x, 3, [256, 256, 512], stage=3, block='a', alpha=alpha)
    x = identity_block(x, 3, [256, 256, 512], stage=3, block='b', alpha=alpha)
    y3 = x

    # Block 4
    x = conv_block(x, 3, [512, 512, 1024], stage=4, block='a', alpha=alpha)
    x = identity_block(x, 3, [512, 512, 1024], stage=4, block='b', alpha=alpha)
    y4 = x

    # Block 5
    x = conv_block(x, 3, [1024, 1024, 2048], stage=5, block='a', alpha=alpha)
    x = identity_block(x, 3, [1024, 1024, 2048], stage=5, block='b', alpha=alpha)
    y5 = x
    ...
    model = Model(inputs, o, name='resnet50')

    return model, rows, cols


if __name__ == '__main__':
    m, rows, cols = ResNet50_like(alpha=0.25)
    print(m.summary())