...
def _conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2, 2), dilation_rate=(1, 1)):
    #   filter array
    ...
    # Block 0
    x = Conv2D(filters1, kernel_size, strides=strides, padding='same', dilation_rate=dilation_rate, name=conv_name_base + '_0')(input_tensor)
    x = BatchNormalization(name=bn_name_base + '_0')(x)
    x = Activation('relu', name=relu_name_base + '_0')(x)

    # Block 1
    x = Conv2D(filters2, kernel_size, padding='same', dilation_rate=dilation_rate, name=conv_name_base + '_1')(x)
...
def _identity_block(input_tensor, kernel_size, filters, stage, block):
    ...
    x = Conv2D(filters1, kernel_size, padding='same', name=conv_name_base + '_0')(input_tensor)
    x = BatchNormalization(name=bn_name_base + '_0')(x)
    x = Activation('relu', name=relu_name_base + '_0')(x)

    x = Conv2D(filters2, kernel_size, padding='same', name=conv_name_base + '_1')(x)
    x = BatchNormalization(name=bn_name_base + '_1')(x)
...
def build_resnet_blocks(input_tensor, kernel_size, filters, group=1, strides=(2, 2), _identities=2):
    if K.image_data_format() == 'channels_last':
        bn_axis = 3
    else:
        bn_axis = 1
    ...
    if group == 1:
        ...
        x = _conv_block(x, kernel_size, filters, stage=group, block=0, strides=strides)
        for block in range(_identities):
            x = _identity_block(x, kernel_size, filters, stage=group, block=block+1)
    return x


def ResNet18(include_top=False, input_tensor=None, input_shape=(para.img_rows, para.img_cols, para.channels)):
    ...
    #   Group 1
    #   =======
    x = build_resnet_blocks(img_input, kernel_size=(7, 7), filters=[64], group=1, strides=(2, 2), _identities=0)
    y1 = x

    #   =======
    #   Group 2
    #   =======
    x = build_resnet_blocks(x, kernel_size=(3, 3), filters=[64, 64], group=2, strides=(1, 1), _identities=1)
    y2 = x

    #   =======
    #   Group 3
    #   =======
    x = build_resnet_blocks(x, kernel_size=(3, 3), filters=[128, 128], group=3, strides=(2, 2), _identities=1)
    y3 = x

    #   =======
    #   Group 4
    #   =======
    x = build_resnet_blocks(x, kernel_size=(3, 3), filters=[256, 256], group=4, strides=(2, 2), _identities=1)
    y4 = x

    #   =======
    #   Group 5
    #   =======
    x = build_resnet_blocks(x, kernel_size=(3, 3), filters=[512, 512], group=5, strides=(2, 2), _identities=1)
    y5 = x
...
def ResNet34(include_top=False, input_tensor=None, input_shape=(para.img_rows, para.img_cols, para.channels)):
    ...
    #   Group 2
    #   =======
    x = build_resnet_blocks(x, kernel_size=(3, 3), filters=[64, 64], group=2, strides=(1, 1), _identities=2)
    y2 = x

    #   =======
    #   Group 3
    #   =======
    x = build_resnet_blocks(x, kernel_size=(3, 3), filters=[128, 128], group=3, strides=(2, 2), _identities=3)
    y3 = x

    #   =======
    #   Group 4
    #   =======
    x = build_resnet_blocks(x, kernel_size=(3, 3), filters=[256, 256], group=4, strides=(2, 2), _identities=5)
    y4 = x

    #   =======
    #   Group 5
    #   =======
    x = build_resnet_blocks(x, kernel_size=(3, 3), filters=[512, 512], group=5, strides=(2, 2), _identities=2)
    y5 = x
